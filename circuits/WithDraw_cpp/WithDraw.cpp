...
uint get_main_input_signal_no() {return 68;}

uint get_total_signal_no() {return 80410;}

uint get_number_of_components() {return 2402;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 44180;}
...
uint sub_component_aux;
uint index_multiple_eq;
Fr_sub(&expaux[3],&circuitConstants[1],&signalValues[mySignalStart + 4]); // line circom 22
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 22
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[0]); // line circom 22
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 2]); // line circom 23
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 23
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 23
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 1];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 3]); // line circom 24
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 24
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 3]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 0; i++){
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[96]); // line circom 36
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 38
if(Fr_isTrue(&expaux[0])){
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 38
// end load src
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 36
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[96]); // line circom 36
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[63]].signalStart + 0]); // line circom 47
...
uint csoffset = mySignalStart+23447;
for (uint i = 0; i < 1; i++) {
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 1],&signalValues[mySignalStart + 1]); // line circom 39
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 39. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
uint cmp_index_ref = 1;
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[96]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
