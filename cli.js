...
let groth16,  circuit, proving_key, senderAccount
let PRIVATE_KEY
...
async function deposit() {
    ...
    await tornado.methods.deposit(deposit.commitment.toString('hex')).send({ ETH_AMOUNT, from: senderAccount, gas: 2e6 }) // TODO: Fix this
    await printETHBalance({ address: tornado._address, name: 'Khang and Phu' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    return noteString
}
